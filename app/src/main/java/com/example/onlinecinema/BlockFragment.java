...
public class BlockFragment extends Fragment {
    ...
    public BlockFragment(String tagFragment) {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        ImageView arrowBackButton = view.findViewById(R.id.arrowBackButtonBlock);
        Button actionButton = view.findViewById(R.id.actionButtonBlock);
        TextView userInfoText = view.findViewById(R.id.userInfoTextBlock);
        EditText userInfoField = view.findViewById(R.id.userInfoFieldBlock);
        EditText passwordField = view.findViewById(R.id.passwordFieldBlock);
        Button searchUserButton = view.findViewById(R.id.searchUserButtonBlock);
        ProgressBar progress = view.findViewById(R.id.progressSearchUserBlock);
                            ...
                            if (response.isSuccessful()) {
                                ...
                                requireActivity().runOnUiThread(new Runnable() {
                                    ...
                                    public void run() {
                                        ...
                                        if (user != null) {
                                            if (user.getActive()) {
                                                actionButton.setText("Заблокировать");
                                            } else {
                                                actionButton.setText("Разблокировать");
                                            }
                                        }
                                    }
                                });
            ...
            public void onClick(View view) {
                ...
                if (securePassword(password).equals(UserRepo.getCurrentUser().getValue().getPassword())) {
                    if (user != null && user.getIdRole() > 1) {
                        ...
                        Request request;
                        if (user.getActive()) {
                            request = client.createPutRequest("/users/block/" + user.getId(), body);
                        } else {
                            request = client.createPutRequest("/users/unblock/" + user.getId(), body);
                        }
                        client.getClient().newCall(request).enqueue(new Callback() {
                            @Override
                            public void onFailure(@NonNull Call call, @NonNull IOException e) {
